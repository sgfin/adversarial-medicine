...
class Dermatology_image_loader(object):
    def __init__(self):
        self.X_test = np.load('data/val_test_x_preprocess.npy')
        self.y_test = np.load('data/val_test_y.npy')
        self.X_train = np.load('data/val_train_x_preprocess.npy')
        self.y_train = np.load('data/val_train_y.npy')
...
class ModelContainer():
  ...
  def inference_batch(self, images=None, target_ys=None, scale=None, n_images=3):
    ...
    if images is None:
      images, random_indices, true_labels = image_loader.get_test_images(n_images)
    ...
    return loss_per_example_arr, ps_arr, ims_arr, random_indices
...
def report(model, target_ys, n_images=3, n_show=5, scale=0.5, predict_original=False):
    """
    This function applies the patch, run prediction of patched (and unpatched) images, calculates the attack success rate, and plots the resulting patched images.

    Args:
        model: Model to be used for prediction (ModelContainer object)
        target_ys: One hot encoded target label
        n_images: Number images to evaluate the performance
    ...
    # can run the inference_batch with multiple of BATCH_SIZE only
    n_images = n_images // BATCH_SIZE * BATCH_SIZE
    # random_indices are the indices for the batch being reported
    loss_per_example, probs_patched_images, patched_imgs, random_indices = model.inference_batch(scale = scale, target_ys = target_ys, n_images = n_images)
    ...
    target_y = np.argmax(target_ys, axis=1)[0]
    winp = (np.argmax(probs_patched_images, axis=1) == target_y).sum() / n_images

    for i in range(n_show):
        show_patched_image(patched_imgs[i], probs_patched_images[i], probs_original_images[i], true_labels[i], random_indices[i])

    return probs_patched_images, probs_original_images, random_indices, true_labels, winp

  
def report_opp(model, target_label, target_ys=None, n_show=5, scale=0.5, predict_original=False):
    """
...
def attack(model, patch, n_images = 16, target_label = 1, n_show = 5, scale = 0.4, predict_original = False):
    """
    This function attacks the target model with the given patch
    ...
    # Report 
    target_ys = gen_target_ys(target_label=target_label, batch_size = n_images)
    probs_patched_images, probs_original_images, random_indices, true_labels, winp = report(model, target_ys, n_images = n_images, n_show = n_show, scale = scale,  predict_original = predict_original)

    return probs_patched_images, probs_original_images, random_indices, true_labels, winp

  
def attack_opp(model, patch, target_label = 1, n_show = 5, scale = 0.4, predict_original = False):
    """
    This function attacks the target model with the given patch. This function uses all the images.
    ...
    # target_ys = gen_target_ys(target_label=target_label, batch_size = n_images)
    probs_patched_images, probs_original_images, indices, true_labels, winp = report_opp(model, target_label = target_label, n_show = n_show, scale = scale,  predict_original = predict_original)

    return probs_patched_images, probs_original_images, indices, true_labels, winp

  
def attack_combined(model, patch_for_0, patch_for_1, n_show=1, scale=0.4, predict_original=False):
    """
    This function is a wrapper for attack_opp. It runs attack_opp twice with target 1 and target 0, then combine the results.
    ...
    probs_patched_images0, probs_original_images0, indices0, true_labels0, winp0 = attack_opp(model, patch_for_0, target_label=0, n_show=n_show, scale=scale, predict_original=predict_original)

    # Attack with target_label = 0
    probs_patched_images1, probs_original_images1, indices1, true_labels1, winp1 = attack_opp(model, patch_for_1, target_label=1, n_show=n_show, scale=scale, predict_original=predict_original)
...
if __name__ == "__main__":
    ...
    file_name = './patches/resnet1_patch_target1.npy'
    resnet1_patch = np.load(file_name)
    target_label = 1
    
    # Blackbox attack (change combinations of model object, patch file, target label)
    probs_patched_images, probs_original_images, random_indices, true_labels, winp = attack(resnet2, resnet1_patch, target_label=1, n_show=5, scale=0.4, predict_original=False)
    
    # # Whitebox attack
    # probs_patched_images, probs_original_images, random_indices, true_labels, winp = attack(resnet1, resnet1_patch, target_label=1, n_show=5, scale=0.4, predict_original=False)    
